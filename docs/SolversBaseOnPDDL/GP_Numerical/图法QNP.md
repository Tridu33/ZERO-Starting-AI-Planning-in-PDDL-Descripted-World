...
为便于状态的枚举过程做到不重不漏，并简化后续图示，

约定一种"状态 $s$ → 整数 $\in \{0, 1, 2, \ldots, 2^{f+v} - 1\}$"的映射方式，即状态码。
...
- $a_4$, pick-x（拿起 $x$ 石头），**关键动作**，其 precondition 与 effect 之间存在一对一映射关系。
可将 $\boldsymbol{g^{\ldots}} \to \ldots \to g' \to G$ 状态链合并表示为 $g^{\ldots}$ 状态，即目标状态 $G$ 的"等价态 $s$"，从而减少状态数量，避免不必要的排序。
