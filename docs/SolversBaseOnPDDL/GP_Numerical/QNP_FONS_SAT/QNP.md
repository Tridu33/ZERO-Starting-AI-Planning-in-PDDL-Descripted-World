...
2. 然后，检验由现成 FOND 规划器所获得的解 P 是否**终止**。这最后一步的必要性源于如下理论观察：FOND 问题 P 中的不确定性本身是 unfaire，但在条件意义上是 faire——若数值变量 X 的增加不超过有限次数，则 X 的无限定性减量最终必然使表达式 X=0 成立（Bonet, De Giacomo, Geffner & Rubin, 2017）。
...
由 FOND 问题 P = ⟨F, I, O, G⟩ 确定的状态模型 S(P) 是一个元组 S(P) = ⟨S, s₀, Act, A, F, S_G⟩，其中 **状态转移函数 F** 是非确定性的，它将动作 a 和状态 s 映射到可能的后续状态的非空集合 F(a, s)。与经典情形类似，非确定性转移函数 F 以分解形式给出。具体而言，每一动作 a 由多个效果分支 Eff₁ | ... | Effₙ 构成（当 n=1 时为确定性动作），F(a, s) 中的每个可能结果状态 s' 对应于为 a 的每个非确定性效果选择一个 Effᵢ 的结果 $^2$。
...
转换步骤说明：
1. 文字 X=0 和 X>0 经由数值变量 X 的消去（Elimination）处理：X=0 表示为命题符号 $P_X=0$，而 X>0 表示为 $\neg P_X=0$。直观理解即为"非零即真"，此处将其视作一种标识 True，建模目标为令控制变量 X=0。
2. $Inc(x)$ 代表确定性的后续命题效果 X>0，如上图中动作 b 的箭头指向所示，其后续均为 X>0。
3. $Dec(x)$ 代表非确定性的后续命题效果 `X>0|X=0`，即 $P_X=0 | \neg P_X=0$。仍以上图为例，动作 a 的箭头有可能指向 X>0 或 X=0 两种可能结果。
