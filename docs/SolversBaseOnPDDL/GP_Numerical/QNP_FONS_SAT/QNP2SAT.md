...
首先枚举状态空间内的所有原子命题 F 和数值变量 V，
基于此衍生生成合取范式（CNF）子句，
调用 SAT 求解器求解，找出所有为真的原子命题 F 和数值变量 V，
最后筛选出那些为真的状态子句并打印，即构成一个连续的求解步骤序列。
...
在 2 个状态的搜索空间下：s₀、s_G 与所有动作中的缺省值组合，得到变量集合。
类似地，在 3 个状态的搜索空间下：s₀、s_G、s₁ 与所有动作中的缺省值组合，得到变量集合。
