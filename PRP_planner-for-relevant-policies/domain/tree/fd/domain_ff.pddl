(define (domain trees_alt)
	...
	(:action copy-right
		...
		:precondition (and 
			...
			(not (assignment ?var2 ?node)))
		:effect (and(assignment ?var2 ?child))
		; :effect (andIf you take the inverse memory and only keep one memory predicate, you can't access the right side after accessing the left side not(assignment ?var1 ?parent))(assignment ?var2 ?child))
	)
	...
	; 	(exists
	; 		(?node - node)
	; 		(and (assignment ?var ?node) (internal ?node)))
	; )
